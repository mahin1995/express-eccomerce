    ...
    return createTablesSafely(knex)([
      {
        name: "order",
        schema(orderTable) {
          orderTable.increments("id").unsigned().primary();
          orderTable.string("user");
          orderTable.json("product");
          orderTable.string("name");
          orderTable.string("phone");
          orderTable.string("address");
